...
- `coordinator_role_ids` ("Regional/Group Coordinators") and `region_role_ids` ("Region Roles") — multi-selects on Role Setup, both restricted to roles starting with `EVENT_ROLE_PREFIXES` and both validated through `EventRoleSetupForm._clean_prefixed_role_ids`
