...
- **The results heatmap lays each time slot out in rider lanes** (`_results_row_in_lanes`, `apps/events/views.py`): one line per rider free at that time on any day shown, repeated in the same order in every open cell and blank where the rider is not free, so a rider reads straight across the row. A lane is one fixed-height line — nothing in it may wrap, or every lane below it moves out of step (an over-long name is cut short instead) — and a scheduled race's badge reserves a line in every open cell of its row. Guarded by `apps/events/test_availability_rider_lanes.py`. A rider picked for a race (not a substitute) gets 🏁 before their name in every slot of that day — the race's day in the **display** timezone, i.e. the column its badge sits in, not its UTC date — so each cell holds its own copy of the rider entries (`racing` is per day). Guarded by `apps/events/test_availability_racing_flag.py`
