...
- Key endpoints:
    ...
    - `POST /api/dbot/sync_guild_members` - Bot-driven sync fallback; the platform now drives this itself (see Guild Member Sync). Optional `observed_at`: a departure stamped at or after it is not cleared (`rejoin_deferred`)
    - `POST /api/dbot/member_left/{discord_id}` - The bot's member-remove event: records that one departure at once (`record_member_departure`); id must be ASCII digits (else 400); body `{}` or optional `username`/`display_name`/`avatar_hash`/`is_bot`
...
- **Leave report** (not a sync): `POST /api/dbot/member_left/{discord_id}` → `apps/accounts/services.py:record_member_departure` stamps one departure at once and files the ticket. A departed row keeps its departure with the stamp moved up to now (`already_departed`, no ticket), so a sync list read before a second leave cannot clear it. With no row it creates a departed one (never "never seen"), with a ticket only if some account holds the id. Either sync clears a stamp only when its list was read after it (`date_left < observed_at`), else counts `rejoin_deferred`.
